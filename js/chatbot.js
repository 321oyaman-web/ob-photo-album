/* ========================================
   ...
   2026年8月29日の Sophia ESS Reunion 2026、1日分の写真を会話から探せるようにする。
   ルールベース（API 費用ゼロ）で動く。
======================================== */
...
(function () {
  ...
  function respond(rawText) {
    ...
    if (photos.length === 0 && !has(text, ['使い方', 'ヘルプ', '追加', 'アップロード'])) {
      botSay(
        'まだ写真が1枚も登録されていません。\n' +
        '8月29日の Sophia ESS Reunion 2026 が終わったら、幹事が写真をアップロードします。もう少しお待ちください。',
        [],
    ...
    if (has(text, ['こんにちは', 'こんばんは', 'おはよう', 'はじめまして', 'よろしく'])) {
      botSay(
        'こんにちは！ Sophia ESS Reunion 2026 のアルバム係です。\n' +
    ...
    if (has(text, ['使い方', 'ヘルプ', 'help', 'できること', '何ができ', 'なにができ'])) {
      botSay(
        'このアルバムは、2026年8月29日に開催された Sophia ESS Reunion 2026 の写真をまとめたものです。\n\n' +
        '【写真を探す】\n' +
        '・「集合写真」「二次会」など場面で絞り込み\n' +
        '・「乾杯」などキーワードでも検索できます\n' +
        '・「最初の方」「最後の方」で時間帯を指定\n' +
    ...
    if (has(text, ['何枚', 'なんまい', '枚数', 'いくつ', '合計', '全部で'])) {
      const summary = scenes
        ...
        .join('\n');
      botSay(
        `Sophia ESS Reunion 2026 の写真は、全部で ${photos.length} 枚です。\n\n【シーン別】\n${summary}`,
        [],
        defaultQuickReplies()
      );
      return;
